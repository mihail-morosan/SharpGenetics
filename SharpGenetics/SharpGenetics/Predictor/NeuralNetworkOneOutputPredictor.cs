...
{
    [DataContract]
    public class NeuralNetworkOneOutputPredictor : ResultPredictor
    {
        [DataMember]
        ...
        {
            if (NetworkTrainingData.Count() == 0)
            {
                NetworkAccuracy = -1;
                return;
            }
        ...
        {
            TrainNetwork(RunMetrics.AverageFitnesses.LastOrDefault().Value);
